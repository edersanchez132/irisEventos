<template>
    <Transition name="modal-animation">
        <div v-if="modalActiveError" class="mainModal">
            <div @click="close" class="modalOverlay" v-on:click="TraceEvent('Model-Event-Close-Error-', $store.state.id)"></div>
            <div class="modalContent iris" :class="randomNumber">
                <div @click="close" class="closeButton" v-on:click="TraceEvent('Model-Event-Close-Error-', $store.state.id)">
    ...
    methods: {
        ...
        TraceEvent(description, user_id) {
            ...
            if (description && user_id) {
                UserService.getEventLogTraceContent(id, user_id).then((response) => {
                   
                }).catch(error => {
                    console.log('getModelEventErrorLogTraceContent', error)
                })
            }
        },
